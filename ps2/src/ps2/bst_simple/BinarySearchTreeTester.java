...
public class BinarySearchTreeTester {
	...
	// helper function to find max element in a a list
	...
	public void testEmptyTreeIllegalOperations() {
		...
		for (int i = 0; i < 100; i++) {
			Assert.assertEquals("tree should not have keys", tree.contains(i),
					false);
			try {
				tree.find(i);
			...
			for (int i = 0; i < num_integers; i++) {
				...
				if (inserted_integers.contains(i)) {
					if (Math.random() > 0.5) {
						...
						try {
							tree.find(i);
							Assert.assertEquals("find should throw exception",
									true, false);
						} catch (ItemNotFoundException e) {
							// should come here.
						}
	...
	public void testDuplicationOperation() {
		...
		for (Integer i : inserted_integers) {
			if (Math.random() > 0.5) {
				try {
					tree.insert(i);
					Assert.assertEquals("insert should throw exception", false,
							true);
	...
	public void TestRepInvariant()
	{
		...
		{
			bst.checkRep();
			Assert.assertTrue(true); //should get here
		}
		catch(RuntimeException re)
		{
			//should not get here!
			Assert.assertEquals("CheckRep should not throw exception!", false, true);
		}
	}
}
